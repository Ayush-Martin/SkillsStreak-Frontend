...
export const getAdminQuizSubmissionsApi = createAsyncThunk<
  IResponse,
  {
    page: number;
    search: string;
...
>(
  "adminQuizSubmissions/getQuizSubmissions",
  async ({ page, search, size }, { rejectWithValue }) => {
    try {
      const response = await appApi.get(
        `/admin/quizSubmissions?search=${search}&page=${page}&size=${size}`
      );
