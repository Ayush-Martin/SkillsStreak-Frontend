...
import { IChat, IChatMessage } from "@/types/chatType";
...
const ChatNew = () => {
  ...
  useEffect(() => {
    ...
    socket.on(
      ...
      ({ chatId, lastMessage }: { lastMessage: string; chatId: string }) => {
        setChats((prevChats) => {
    ...
    <UserLayout>
      <div className="relative flex">
        ...
        <div className="w-full mt-10 ml-0 text-white md:ml-64 md:mt-0 p-5">
          <div className="w-full h-[600px] border border-app-border flex rounded-md bg-[#111827]">
            <ChatSidebar
              isOpen={isOpen}
              chats={chats}
