...
const UserSubscriptionPage = () => {
  ...
  const calculateUpgradePrice = (newPrice: number) => {
    if (!subscriptionDetail) return newPrice;

    const totalDays =
      (new Date(subscriptionDetail.endDate).getTime() -
        new Date(subscriptionDetail.startDate).getTime()) /
      (1000 * 60 * 60 * 24);

    const remainingDays =
      (new Date(subscriptionDetail.endDate).getTime() - Date.now()) /
      (1000 * 60 * 60 * 24);

    const dailyCurrentPrice = subscriptionDetail.amount / totalDays;
    const creditAmount = dailyCurrentPrice * remainingDays;

    return Math.floor(Math.max(newPrice - creditAmount, 0));
  };
      ...
      <section className="max-w-6xl min-h-screen mx-auto mt-10 px-4 pt-12 pb-8">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-5xl mx-auto">
          {subscriptionPlans.map((plan) => {
            const isCurrentPlan =
              subscriptionDetail?.subscriptionPlanId === plan._id;
            const currentPlan = subscriptionPlans.find(
              (p) => p._id === subscriptionDetail?.subscriptionPlanId
            );
            const isUpgrade =
              subscriptionDetail &&
              currentPlan &&
                ...
                <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-gray-700 hover:border-blue-500/50 rounded-2xl p-8 h-full flex flex-col relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    {isCurrentPlan && (
                      <button
                        disabled
                        className="w-full bg-gray-600 text-white font-semibold py-3 px-6 rounded-xl cursor-not-allowed"
                      >
                        Current Plan
                      </button>
                    )}
                    ...
                    {isUpgrade && (
                      <button
                        ...
                        className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300"
                      >
                        Upgrade for ₹
                        {calculateUpgradePrice(plan.price).toFixed(2)}
                      </button>
                    )}
                    ...
                    {!isCurrentPlan &&
                      ...
                      (currentPlan?.price || 0) < plan.price && (
                        <button
                          onClick={() => handleSubscribe(plan._id)}
                          className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300"
                        >
                          Subscribe Now
                        </button>
                      )}
